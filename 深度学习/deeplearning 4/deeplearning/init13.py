...
STATE_SIZE = 8
REPEATS = 6
SAVE_PATH = 'model/init13/mymodel'
...
class Tensors:
    def __init__(self, batch_size):
        ...
        for i in range(REPEATS):
            ...
            with tf.variable_scope('rnn1', reuse=reuse):
                x = _my_fc(input, STATE_SIZE, 'fc1')
                x = tf.nn.relu(x)
                state = tf.concat([state, x], axis=1)
                output = _my_fc(state, 1, 'out1')
