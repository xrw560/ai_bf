...
# plt.plot(ln_x_test,lr_y_test_hat,'b-',lw=2,label=u'Linear回归，$R^2$=%.5f'%lr_score)
# plt.plot(ln_x_test,lasso_y_test_hat,'y-',lw=2,label=u'Lasso回归，$R^2$=%.5f'%lasso_score)
# plt.plot(ln_x_test,ridge_y_test_hat,'c-',lw=2,label=u'Ridge回归，$R^2$=%.3f'%ridge_score)
# plt.plot(ln_x_test,y_test_hat,'g-',lw=4,label=u'回归决策树预测值,$R^2$=%.5f'%score)
# plt.xlabel(u"数据编码")
# plt.ylabel(u"租赁价格")
...
for t in range(3):
    ...
    print(t, "score值", gscv.best_score_, u'最优参数列表：', gscv.best_params_)
...
print(u"正确率：",decision3.score(x_test3,y_test3))
...
plt.grid(True)
plt.title(u"决策树层次太多导致的拟合问题",fontsize=18)
plt.show()
