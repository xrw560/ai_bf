...
class Tensors:
    def __init__(self, gpus):
        ...
        self.clip = [tf.assign(var, tf.clip_by_value(var, -C, C)) for var in tf.trainable_variables() if var.name.startswith('mnist/generator')]
    ...
    def _build_tensors(self):
        ...
        z_disc_loss = tf.reduce_mean(x_result) - tf.reduce_mean(z_result)
        z_disc_optimizer = tf.train.RMSPropOptimizer(self.lr)
        var_list = [var for var in tf.trainable_variables() if var.name.startswith('mnist/discriminator')]
        ...
        z_gen_loss = tf.reduce_mean(z_result)
        z_gen_optimizer = tf.train.RMSPropOptimizer(self.lr)
        ...
        for i in range(epoches):
            ...
            for j in range(steps):
                for _ in range(5):
                    ...
                    ts = self.tensors
                    self.session.run(ts.z_disc_minimize, feed_dict=feed_dict)

                su, _ = self.session.run([ts.summary, ts.z_gen_minimize], feed_dict=feed_dict)
            ...
            print('%d/%d: Model is saved into %s' % (i+1, epoches, SAVE_PATH), flush=True)

    def predict_gan(self, batch_size):
        samples, _ = self.ds.test.next_batch(batch_size)
        ...
        ts = self.tensors
        fake_ss = self.session.run(ts.z_fake_s, feed_dict=feed_dict)
        for fake_s in fake_ss:
            for fake in fake_s:
                my_print(fake)
...
def my_print(sample):
    ...
    for y in range(28):
        for x in range(28):
            print(' ' if sample[y][x] < 0.05 else 'o', end='')
        print(flush=True)
    print('-' * 100)


def get_gpus():
    temp = os.getenv('CUDA_VISIBLE_DEVICES')
    print('CUDA_VISIBLE_DEVICES = %s' % temp)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--batch_size", help="the batch size, default to 128", type=int, default=128)
    parser.add_argument("--epoches", help="the epoches, default to 50", type=int, default=50)
