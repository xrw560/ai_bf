...
STATE_SIZE = 8
REPEATS = 6
SAVE_PATH = 'model/init14/mymodel'


class Tensors:
    def __init__(self, batch_size):
        lstm = MyLSTM(STATE_SIZE, 1)

        state = lstm.zero_state(batch_size)
        output = lstm.zero_output(batch_size)
        inputs = []

        for i in range(REPEATS):
            input = tf.placeholder(dtype=tf.float32, shape=[None, 1])
            inputs.append(input)
            output, state = lstm(input, output, state)
...
class Init:
    ...
    def train(self, epoches=5000):
        ...
        file_writer = tf.summary.FileWriter('log14', graph=self.graph)
        step = 0
