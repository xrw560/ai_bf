...
parser.add_argument('-e', '--evaluate', default=1, dest='evaluate', action='store_true',
...
# Validate dataset
assert args.dataset == 'cifar10', 'Dataset can only be cifar10 or cifar100.'
...
def main():
    ...
    if args.arch.startswith('resnext'):
        ...
        model = getattr(models, args.arch)(num_classes=num_classes, depth=args.depth)
        # model = models.__dict__[args.arch](
        # num_classes=num_classes,
        # depth=args.depth,
        # )
        ...
        # model1 = models.vgg11(num_classes=num_classes)
    ...
    if args.resume:
        # Load checkpoint.checkpoint = torch.load(args.resume, map_location='cpu')
        print('==> Resuming from checkpoint..')
        assert os.path.isfile(args.resume), 'Error: no checkpoint directory found!'
        args.checkpoint = os.path.dirname(args.resume)

        checkpoint = torch.load(args.resume, map_location='cpu') if not use_cuda else torch.load(args.resume)
